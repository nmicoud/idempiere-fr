...
/**
 *  State and data access helper for {@link WLFRAcctViewer}
 *
 *  @author Nicolas Micoud - TGI
 *  		July 27, 2007
 */
public class WLFRAcctViewerData
{
	/** Window              */
	public int WindowNo;
	...
	/** Display Lines With Debit=Credit=0	*/
	protected boolean displayLines0 = false;
	
	/** Grouper les �critures par document/date/compte/bpartner (en faisant abstraction des autres dimensions) */
	protected boolean groupByAccount = false;
	...
	/**	Logger			*/
	private static final CLogger log = CLogger.getCLogger(WLFRAcctViewerData.class);

	public String Lettrage = "";	// Ajout nico 300911
	public Timestamp LettrageDate = null;	// Ajout nico 300911
	 ...
	 */
	public WLFRAcctViewerData (Properties ctx, int windowNo, int ad_Client_ID, int ad_Table_ID)
	{
		WindowNo = windowNo;
	...
	protected RModel query()
	{
		...
		if (!displayLines0) {
			if (whereClause.length() > 0)
				whereClause.append(" AND ");
			whereClause.append("(").append(RModel.TABLE_ALIAS).append(".AmtAcctDR != 0 OR ").append(RModel.TABLE_ALIAS).append(".AmtAcctCR != 0)");
		}
		...
		else
		{
			...
			// Lettrage
			if (!Util.isEmpty(Lettrage)) {
				if (whereClause.length() > 0)
					whereClause.append(" AND ");

				if (LettrageDate == null) { // pas de date pr�cis�e
					whereClause.append(RModel.TABLE_ALIAS).append(".LFR_MatchCode IS ");
					if (Lettrage.equals("Y"))
						whereClause.append("NOT ");
					whereClause.append("NULL");
				}
				else {
					if (Lettrage.equals("Y"))
						whereClause.append("TRUNC(").append(RModel.TABLE_ALIAS).append(".LFR_ReconciliationDate, 'DD') <= ").append(DB.TO_DATE(LettrageDate));
					else {
						whereClause.append("(").append(RModel.TABLE_ALIAS).append(".LFR_MatchCode IS NULL OR ");
						whereClause.append("TRUNC(").append(RModel.TABLE_ALIAS).append(".LFR_ReconciliationDate, 'DD') > ");
						whereClause.append(DB.TO_DATE(LettrageDate)).append(")");
					}
				}
			}
			// Fin lettrage
...
//			orderClause.append(RModel.TABLE_ALIAS).append(".Fact_Acct_ID");
				...
				RModel rm = getRModel(); // on cr�e le RModel avant de d�finir la Order Clause - besoin de savoir si on aura la colonne Fact_Acct_ID
		
		if (orderClause.length() == 0) {
			if (rm.getColumnIndex("Fact_Acct_ID") < 0) // Fact_Acct_ID n'est pas pr�sent sur LFR_FactAcct_GroupBy_Account
				orderClause.append(RModel.TABLE_ALIAS).append(".DateAcct, ").append(RModel.TABLE_ALIAS).append(".AccountValue");
			else // on trie d'abord par date et ensuite par ID
				orderClause.append(RModel.TABLE_ALIAS).append(".DateAcct, ").append(RModel.TABLE_ALIAS).append(".Fact_Acct_ID");
		}
...
//		rm.setFunction("AmtAcctCr", RModel.FUNCTION_SUM);
		rm.setFunction("CASE WHEN AmtAcctDr = 0 THEN NULL ELSE AmtAcctDr END", RModel.FUNCTION_SUM);
		rm.setFunction("CASE WHEN AmtAcctCr = 0 THEN NULL ELSE AmtAcctCr END", RModel.FUNCTION_SUM);
	...
	//	RModel rm = new RModel("Fact_Acct");
		RModel rm = null;
		if (!groupByAccount)
			rm = new RModel("LFR_FactReconciliation_v"); // Fact_Acct dans le standard
		else
			rm = new RModel("LFR_FactAcct_GroupBy_Account");
		...
		//  Main Info
		rm.addColumn(new RColumn(ctx, "AmtAcctDr", DisplayType.Amount, "CASE WHEN AmtAcctDr = 0 THEN NULL ELSE AmtAcctDr END"));
		rm.addColumn(new RColumn(ctx, "AmtAcctCr", DisplayType.Amount, "CASE WHEN AmtAcctCr = 0 THEN NULL ELSE AmtAcctCr END"));
		rm.addColumn(new RColumn(ctx, "GL_Category_ID", DisplayType.TableDir));
		...
		if (groupByAccount)
			...
			rm.addColumn(new RColumn(ctx, "XXA_Lettrage", DisplayType.String, "LFR_MatchCode"));
